...
class EnsemblspiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        try:
            ...
            elif r_allele1 in ref_alleles and r_allele1 not in ALT_alleles:
                riskAllele.extend()
            elif r_allele1 in ALT_alleles and r_allele2 in ALT_alleles:
                riskAllele.extend(ALT_alleles)
                n_alleles.extend(ref_alleles)
            elif r_allele1 in ALT_alleles and r_allele2 not in ALT_alleles:
                riskAllele.append(r_allele1)
                n_alleles.extend(ref_alleles).extend(ALT_alleles.remove(r_allele1))
            elif r_allele1 not in ALT_alleles and r_allele2  in ALT_alleles:
                riskAllele.append(r_allele2)
                n_alleles.extend(ref_alleles).extend(ALT_alleles.remove(r_allele2))
